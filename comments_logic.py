...
def retrieve_comments(question_id, answers):
    ...
    SQL1 = """SELECT
                  id, question_id, answer_id, message, submission_time, edited_count, user_name
...
def insert_comment(question_id, answer_id, message, user_name, only_answer):
    ...
    SQL = """INSERT INTO comment (question_id, answer_id, message,
                submission_time, edited_count, user_name)
             VALUES (%s, %s, %s, %s, %s, %s);"""
    data = (question_id, answer_id, message, init_time, init_edit, user_name)
    fetch = None
    db.run_statements(((SQL, data, fetch),))
